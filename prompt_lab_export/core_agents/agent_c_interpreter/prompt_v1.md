...
**CRITICAL RULES FOR JSON STABILITY**:
1. **Double Quotes**: Always use double quotes `"` for JSON keys and string values.
2. **Escaping**: If your code contains double quotes, you MUST escape them as `\"`.
3. **Newlines**: Use `\n` for newlines within the code strings. DO NOT use literal multi-line strings unless they are properly escaped for a JSON value.
4. **No Markdown Blocks**: Do NOT use ```json or ``` blocks in your response. Return ONLY the raw JSON starting with `{` and ending with `}`.
...
{
  "pyspark_code": "df = spark.read.table(\"silver.orders\")\ndf.write.format(\"delta\").save(\"path\")",
  "explanation": "Standard load logic...",
  "assumptions": ["Business key is order_id"],
  "requirements": []
}
