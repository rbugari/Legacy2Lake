...
export default function CommandPalette({ isOpen, onClose, projects, onToggleLogs }: CommandPaletteProps) {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-start justify-center pt-32 px-4">
            ...
            <div className="w-full max-w-xl bg-[#0a0a0a] border border-white/5 rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-top-4 duration-300">
                {/* Input Area */}
                <div className="flex items-center px-6 py-4 border-b border-white/5 bg-black/40">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-white text-sm outline-none placeholder-gray-600 font-bold"
                    />
                    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/5 border border-white/5 text-[9px] font-black text-gray-500 uppercase tracking-widest">
                        <span>ESC</span>
                    </div>
                </div>
                ...
                <div className="max-h-[400px] overflow-y-auto p-2 custom-scrollbar">
                    ...
                    {filteredActions.length > 0 && (
                        <div className="mb-4">
                            <h4 className="px-4 py-2 text-[10px] font-black text-gray-500 uppercase tracking-widest">Global Commands</h4>
                    ...
                    {filteredProjects.length > 0 && (
                        <div>
                            <h4 className="px-4 py-2 text-[10px] font-black text-gray-500 uppercase tracking-widest">Active Projects</h4>
                    ...
                    {filteredActions.length === 0 && filteredProjects.length === 0 && (
                        <div className="px-6 py-10 text-center">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">No results found for "{query}"</p>
                        </div>
                    )}
                </div>
...
function CommandItem({ active, label, icon, onClick, shortcut }: any) {
    return (
        <button
            onClick={onClick}
            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all ${active ? 'bg-cyan-600 text-white shadow-xl shadow-cyan-600/20 translate-x-1' : 'text-gray-400 hover:bg-white/5'}`}
        >
            <div className="flex items-center gap-4">
                <div className={`p-2 rounded-lg ${active ? 'bg-white/20' : 'bg-white/5 text-gray-500'}`}>
                    {icon}
                </div>
                <span className="text-xs font-black uppercase tracking-widest">{label}</span>
            </div>
            {shortcut && !active && (
                <span className="text-[9px] font-bold text-gray-600 border border-white/5 px-1.5 py-0.5 rounded uppercase tracking-widest">{shortcut}</span>
            )}
        </button>
    );
}
...
function Tip({ icon, label }: any) {
    return (
        <div className="flex items-center gap-1.5 text-[9px] font-black text-gray-600 uppercase tracking-widest">
            <span className="p-0.5 bg-white/5 rounded">{icon}</span>
            <span>{label}</span>
        </div>
    );
}
