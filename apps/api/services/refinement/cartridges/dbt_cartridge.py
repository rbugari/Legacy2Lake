...
try:
    ...
    DbtTestGenerator = None  # Fallback if not available
...
"""
    ...
    def generate_orchestration(self, tables_metadata: List[Dict[str, Any]]) -> str:
        """Generates an Airflow DAG for dbt orchestration."""
        project_name = f"migration_{self.project_id}"
...
with DAG(
    'dag_dbt_{project_name}',
    default_args=default_args,
    description='dbt Orchestration for {project_name}',
    ...
    dbt_run = BashOperator(
        task_id='dbt_run',
        bash_command='dbt run --project-dir /path/to/{project_name} --profiles-dir /path/to/profiles'
    )

    dbt_test = BashOperator(
        task_id='dbt_test',
        bash_command='dbt test --project-dir /path/to/{project_name} --profiles-dir /path/to/profiles'
    )
