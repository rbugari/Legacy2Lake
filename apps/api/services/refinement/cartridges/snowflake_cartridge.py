...
def main(session: Session):
    ...
    try:
        ...
        merge_conditions = [F.col(f"target.{{pk}}") == F.col(f"source.{{pk}}") for pk in pk_columns]
...
"""
    ...
    def generate_bronze_sql(self, table_metadata: Dict[str, Any]) -> str:
        """Generates pure SQL for Bronze layer with smart file format detection."""
        source_path = Path(table_metadata.get("source_path", "unknown"))
        
        # Detect file format from extension
        ext = source_path.suffix.lower() if source_path.suffix else ".csv"
        ...
        csv_opts = ""
        if file_format == "CSV":
            csv_opts = " FIELD_OPTIONALLY_ENCLOSED_BY = '\"' SKIP_HEADER = 1"
...
FILE_FORMAT = (TYPE = '{file_format}'{csv_opts});
"""
...
"""
    ...
    def generate_orchestration(self, tables_metadata: List[Dict[str, Any]]) -> str:
        """Generates orchestration for Snowflake SQL or Snowpark Python targets."""
        if self.get_file_extension() == ".py":
            dag_lines = [
                "# Snowpark orchestration DAG",
                ...
                "",
                "",
                "def run_stage(stage_name: str, table_name: str, **context):",
                "    # Placeholder hook for Snowpark jobs or Snowflake stored procedures.",
                "    print(f'Running {stage_name} for {table_name}')",
                "",
                "",
                f"with DAG(dag_id='utm_snowpark_{self.project_id}', start_date=datetime(2024, 1, 1), schedule='@daily', catchup=False) as dag:",
                "    root = EmptyOperator(task_id='start')",
            ]
            ...
            for table in tables_metadata:
                ...
                slug = name.lower()
                dag_lines.extend([
                    f"    bronze_{slug} = PythonOperator(task_id='bronze_{slug}', python_callable=run_stage, op_kwargs={{'stage_name': 'BRONZE', 'table_name': '{name}'}})",
                    f"    silver_{slug} = PythonOperator(task_id='silver_{slug}', python_callable=run_stage, op_kwargs={{'stage_name': 'SILVER', 'table_name': '{name}'}})",
                    f"    gold_{slug} = PythonOperator(task_id='gold_{slug}', python_callable=run_stage, op_kwargs={{'stage_name': 'GOLD', 'table_name': '{name}'}})",
                    f"    root >> bronze_{slug} >> silver_{slug} >> gold_{slug}",
                ])
