...
try:
    ...
    try:
        ...
        # Fallback if Sprint 9 services not available
        SchemaMetadataService = None
...
class PySparkCartridge(Cartridge):
    ...
    Sprint 9 Enhancement:
        Uses SchemaMetadataService and ParameterExtractor for zero-hardcode generation
        when asset_id is provided in table_metadata.
    """
...
"""
    ...
    def generate_bronze(self, table_metadata: Dict[str, Any]) -> str:
        ...
        Sprint 9 Enhancement:
            If asset_id is present, uses SchemaMetadataService for dynamic column selection.
        """
        source_path = Path(table_metadata.get("source_path", "unknown_source.py"))
        original_code = table_metadata.get("original_code", "")
        
        # Sprint 9: Try to get schema-aware column list
        column_list_code = "*"  # Default: select all
        table_name_dynamic = source_path.stem
        
        if self.tenant_id and SchemaMetadataService and table_metadata.get("asset_id"):
            try:
                import asyncio
                schema_service = SchemaMetadataService(tenant_id=self.tenant_id, project_id=self.project_id)
                
                # Get schema (note: this is async, need to handle carefully)
                # For now, use simple approach - let Agent C handle schema extraction
                # Cartridge will use schema if passed in table_metadata
                if "schema" in table_metadata and "columns" in table_metadata["schema"]:
                    columns = [col["name"] for col in table_metadata["schema"]["columns"] if not col["name"].startswith("_")]
                    column_list_code = ", ".join([f'"{col}"' for col in columns])
                    table_name_dynamic = table_metadata["schema"].get("table_name", source_path.stem)
...
# Generated by Legacy2Lake Architect Agent (Sprint 9: Schema-Aware)
# Source: {source_path.name}
# Target: {{table_name_dynamic}}
...
# Sprint 9: Select columns dynamically (if schema provided)
df_bronze = df_source if 'df_source' in locals() else df
df_bronze = add_ingestion_metadata(df_bronze)

# Write to Delta with schema enforcement and evolution support
# mergeSchema allows new columns, dataChange tracks CDC
target_table = f"{{Config.CATALOG}}.{{Config.SCHEMA_BRONZE}}.{table_name_dynamic}"
...
"""
    ...
    def generate_silver(self, table_metadata: Dict[str, Any]) -> str:
        """
        Generate silver layer code with SCD Type 2 merge.
        ...
        if "schema" in table_metadata and "primary_key" in table_metadata["schema"]:
            schema_pk = table_metadata["schema"]["primary_key"]
            if schema_pk and len(schema_pk) > 0:
                pk_columns = schema_pk
                print(f"[PySparkCartridge Sprint9] Using schema.primary_key: {pk_columns}")
        
        # Validate and normalize PK columns
        pk_columns = self._validate_and_normalize_pk(pk_columns)
