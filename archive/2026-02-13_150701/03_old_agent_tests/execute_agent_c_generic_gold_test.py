...
def execute_test():
    print("TEST: GENERIC-GOLD-01")
    with open("prompt_lab/cartridges/base/gold_layer.md", 'r', encoding='utf-8') as f:
        prompt = f.read()
    
    response = requests.post(f"{API_BASE}/transpile/task",
        json={"node_data": {"layer": "gold", "tech_id": "generic", "cartridge_prompt": prompt},
    ...
    if response.status_code == 200:
        ...
        with open("prompt_lab/TEST_OUTPUT_BASE_GENERIC_GOLD_01.txt", 'w', encoding='utf-8') as f:
            f.write(code)
