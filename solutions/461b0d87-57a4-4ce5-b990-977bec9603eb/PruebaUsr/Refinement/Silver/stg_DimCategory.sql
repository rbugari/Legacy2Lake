...
-- Target: dim_DimCategory
-- Logic: Upsert from Bronze to Silver with deduplication.

MERGE INTO silver_dim_DimCategory AS target
USING (
    SELECT * FROM (
        SELECT *, ROW_NUMBER() OVER (PARTITION BY empid ORDER BY _ingestion_timestamp DESC) as _rn
        FROM bronze_dim_DimCategory
    ) WHERE _rn = 1
) AS source
