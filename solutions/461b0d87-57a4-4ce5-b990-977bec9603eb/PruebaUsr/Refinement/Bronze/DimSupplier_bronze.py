...
# [DISABLED_BY_ARCHITECT]                 empty_df.write.format("delta").mode("overwrite").saveAsTable(table_full_name)
...
# [DISABLED_BY_ARCHITECT]             logger.info(f"OPTIMIZE {gold_table} ZORDER BY ({bk_col}) -- consider clustering on business key")
            # Actual optimize and vacuum are optional operational commands; we include them as suggestions here
            try:
# [DISABLED_BY_ARCHITECT]                 spark.sql(f"OPTIMIZE {gold_table} ZORDER BY ({bk_col})")
            except Exception:
# [DISABLED_BY_ARCHITECT]                 logger.warning(f"OPTIMIZE failed or not applicable for {gold_table}; skipping")
...
"""
        ...
        try:
# [DISABLED_BY_ARCHITECT]             logger.info(f"OPTIMIZE {gold_fact_table} ZORDER BY (orderdate)")
# [DISABLED_BY_ARCHITECT]             spark.sql(f"OPTIMIZE {gold_fact_table} ZORDER BY (orderdate)")
        except Exception:
# [DISABLED_BY_ARCHITECT]             logger.warning(f"OPTIMIZE failed or not applicable for {gold_fact_table}; skipping")
