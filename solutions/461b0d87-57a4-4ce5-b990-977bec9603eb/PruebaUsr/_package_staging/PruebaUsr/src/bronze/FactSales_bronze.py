...
# [DISABLED_BY_ARCHITECT]             df_create.select([c[0] for c in schemas[target_dim_product]]).write.format('delta').saveAsTable(target_dim_product)
...
# [DISABLED_BY_ARCHITECT]         logger.info('Consider executing OPTIMIZE %s ZORDER BY (productid) for query performance', target_dim_product)
...
# [DISABLED_BY_ARCHITECT]             df_fact_ready.limit(0).write.format('delta').saveAsTable(target_fact)
...
# [DISABLED_BY_ARCHITECT]         logger.info('Post-load: consider running OPTIMIZE and VACUUM as per retention policies for %s and %s', target_dim_product, target_fact)
