...
# [DISABLED_BY_ARCHITECT]                 # Use DeltaTable writer via saveAsTable (first-time create). For full layers, we allow write overwrite only for initial bootstrap.
                staging_to_write = staging_with_sk.select(*insert_cols)
                # Ensure table path/format uses Delta managed table in gold_db
# [DISABLED_BY_ARCHITECT]                 staging_to_write.write.format("delta").mode("overwrite").saveAsTable(target_table)
...
# [DISABLED_BY_ARCHITECT]             fact_df.select(*[c for c, _ in target_schemas.get('fact_sales', [])]).write.format("delta").mode("overwrite").saveAsTable(fact_tgt)

        # Post-load optimization hints (manual steps recommended)
# [DISABLED_BY_ARCHITECT]         logger.info("Load complete. Consider running OPTIMIZE and ZORDER for frequently filtered columns on Gold tables.")
