...
-- Target: dim_DimEmployee
-- Logic: Upsert from Bronze to Silver with deduplication.

MERGE INTO silver_dim_DimEmployee AS target
USING (
    SELECT * FROM (
        SELECT *, ROW_NUMBER() OVER (PARTITION BY  AND  ORDER BY _ingestion_timestamp DESC) as _rn
        FROM bronze_dim_DimEmployee
    ) WHERE _rn = 1
) AS source
ON target. AND  = source. AND 
