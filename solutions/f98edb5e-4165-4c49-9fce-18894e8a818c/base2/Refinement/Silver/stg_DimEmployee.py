...
# Read Bronze
df_bronze = spark.read.table(f"{Config.CATALOG}.{Config.SCHEMA_BRONZE}.DimEmployee")

# Deduplicate
df_clean = df_bronze.dropDuplicates(['empid'])

# SCD Type 2 Merge (Upsert)
target_table_name = f"{Config.CATALOG}.{Config.SCHEMA_SILVER}.stg_DimEmployee"

if SparkSession.active.catalog.tableExists(target_table_name) and "empid" != "id":
