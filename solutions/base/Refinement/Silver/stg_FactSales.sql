...
-- Target: dim_FactSales
-- Logic: Upsert from Bronze to Silver with deduplication.

MERGE INTO silver_dim_FactSales AS target
USING (
    SELECT * FROM (
        SELECT *, ROW_NUMBER() OVER (PARTITION BY orderid, productid ORDER BY _ingestion_timestamp DESC) as _rn
        FROM bronze_dim_FactSales
    ) WHERE _rn = 1
) AS source
