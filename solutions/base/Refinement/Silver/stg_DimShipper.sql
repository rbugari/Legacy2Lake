...
-- Target: dim_DimShipper
-- Logic: Upsert from Bronze to Silver with deduplication.

MERGE INTO silver_dim_DimShipper AS target
USING (
    SELECT * FROM (
        SELECT *, ROW_NUMBER() OVER (PARTITION BY shipperid ORDER BY _ingestion_timestamp DESC) as _rn
        FROM bronze_dim_DimShipper
    ) WHERE _rn = 1
) AS source
